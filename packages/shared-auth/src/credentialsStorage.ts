...
function prefKey(suffix: string): string {
  return `${getCredentialsKeyPrefix()}-${suffix}`;
}

const prefRemember = prefKey('remember');
const prefEmail = prefKey('email');
const prefPassword = prefKey('password');
const prefRole = prefKey('role');
